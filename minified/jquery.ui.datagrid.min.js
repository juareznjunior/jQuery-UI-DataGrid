...
this,d=c.getThead()[0].rows[0].cells,i=c.uiDataGridTbody.empty()[0],g,e;c.uiDataGridScroll.scrollTop(0);b.map(a.rows,function(a,f){g=i.insertRow(-1);g.className="ui-state-hover";c.options.rowNumber&&(e=g.insertCell(0),e.className="ui-state-default ui-datagrid-cell-rownumber",e.innerHTML="<div>"+(parseInt(0<c._offset||1)+f)+"</div>");b.map(c.options.mapper,function(c){e=g.insertCell(-1);e.className="ui-widget ui-widget-content";e.style.cssText="text-align:"+d[e.cellIndex].style.textAlign;e.innerHTML=
b.isFunction(c.render)?c.render.call(e,a[c.name]):a[c.name]})});d=i=g=e=c=null},_ajax:function(){""!=this.options.jsonStore.url?("string"===typeof this.options.jsonStore.params?this.options.jsonStore.params=0===this._offset?this.options.jsonStore.params+"&limit="+this.options.limit+"&offset="+v._offset:this.options.jsonStore.params.replace(/(&offset=)(.+)/,"&offset="+this._offset):(h===this.options.jsonStore.params&&(this.options.jsonStore.params={}),this.options.jsonStore.params.limit=this.options.limit,
this.options.jsonStore.params.offset=this._offset),this.options.pagination&&this._disableToolButtons(),b.ajax({type:this.options.ajaxMethod.toLowerCase(),url:this.options.jsonStore.url.replace(/\?.*/,""),data:this.options.jsonStore.params,dataType:"json",context:this,success:function(a){var c=this;if(h!=a.error)return b.isFunction(c.options.onError)?c.options.onError.call(c.element[0]):alert(a.error),!1;if(h===a.num_rows||0==a.num_rows)return!1;if(c.options.pagination){c._totalPages=Math.ceil(a.num_rows/
c.options.limit);var d=0==c._offset?1:c._offset/c.options.limit+1,i=d+" de "+c._totalPages+" ("+a.num_rows+")";b.each(b(c.uiDataGridTfoot[0].rows[0].cells).eq(-1).children(),function(){/span/i.test(this.tagName)?this.innerHTML=i:/data-grid-button-(first|prev)/.test(this.name)?0<c._offset&&this.disabled&&b(this).button("enable"):c._totalPages>d&&this.disabled&&b(this).button("enable")})}c._createRows(a);c=null}})):this._createRows(this.options.jsonStore.data)},render:function(){var a=this;if(a._active())a.resetOffset(),
a.load();else{if(b.isArray(a.options.toolBarButtons)){var c=a.uiDataGridTfoot[0].rows[0].cells[0];b.map(a.options.toolBarButtons,function(d){(function(a){if(b.isFunction(d.fn))this.on("click",function(){d.fn.apply(a.element[0],arguments);b(this).blur()});this.button({icons:{primary:h===d.icon?null:"ui-icon-"+d.icon}});c.appendChild(this[0])}).call(b(f.createElement("button")).html(d.label),a)});c=null}"none"===a.element.css("display")&&a.element.show();a.uiDataGrid.appendTo(a.element);a._createColumns();
var d=a.uiDataGridThead.outerHeight();a.uiDataGridTheadBody.parent().css("marginTop",-d);d=null;a.options.pagination&&(a._createToolButtons()._disableToolButtons(),b(a.uiDataGridTfoot[0].rows[0].cells).eq(-1).on("click","button",function(a){return function(){if(!1===this.disabled){var b=["_",this.name.replace(/data-grid-button-/,""),"Page"];a[b.join("")]();a.load()}}}(a)));a.resize();a.options.autoLoad&&a.load();b.isFunction(a.options.onComplete)&&function(a){setTimeout(function(){a.options.onComplete.call(a.element[0])})}(a)}a=
null;return this},_nextPage:function(){this._offset+=this.options.limit},_prevPage:function(){this._offset-=this.options.limit},_endPage:function(){this._offset=this._totalPages*this.options.limit-this.options.limit},_firstPage:function(){this._offset=0},_tbodyEvents:function(){if(b.isFunction(this.options.onClickRow))this.uiDataGridTbody.off().on("click","tr",function(a){return function(c){b(this).addClass("ui-state-highlight");a.options.onClickRow.call(a,this);var d=a.uiDataGridTbody[0],f=b.data(d,
"rowselected");c.currentTarget!==f[0]&&f.removeClass("ui-state-highlight");b.data(d,"rowselected",b(c.currentTarget))}}(this));return this},_active:function(){return this.element.children(":eq(0)").hasClass("ui-datagrid-container")},resize:function(){this.options.fit&&function(a){a=a.uiDataGrid.outerHeight()-a.element.height();this.style.height=b(this).height()-a+"px"}.call(this.uiDataGridScroll[0],this);return this},getSelectedRow:function(){return b.data(this.uiDataGridTbody[0],"rowselected")},
