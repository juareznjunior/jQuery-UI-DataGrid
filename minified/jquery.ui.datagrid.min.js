...
Array(d.tHead.rows[0].cells.length).join('</td><td class="ui-widget ui-widget-content">&nbsp;')+"</td></tr>").end().appendTo(d.parentNode);c(d[0].tHead).remove();a.options.rowNumber&&(d[0].tBodies[0].rows[0].cells[0].className="ui-state-default ui-datagrid-cell-rownumber")});c(a.uiDataGridTheadBody[0].parentNode.tHead).hide();e=b=a=g=d=k=null},_createRows:function(a,b,e){var d=this,f=d.getThead()[0].rows[0].cells,j=e?d.uiDataGridTbody[0]:d.uiDataGridTbody.empty()[0],h,g,k=e?j.rows.length+1:d._offset+
1,l="local"===b&&d.options.pagination,b=i===a.num_rows?i===a.rows?i===a[0].num_rows?a.length:a[0].num_rows:i===a.rows[0].num_rows?a.rows.length:a.rows[0].num_rows:a.num_rows,a=a.rows||a;!e&&(l&&1<k)&&(a=a.slice(d._offset));e||(d._managePagination(b),d.uiDataGridScrollBody.scrollTop(0));c.each(a,function(a,b){if(!e&&l&&a===d.options.limit)return!1;h=j.insertRow(-1);h.className="ui-state-hover";d.options.rowNumber&&(g=h.insertCell(0),g.className="ui-state-default ui-datagrid-cell-rownumber",g.innerHTML=
"<div>"+(k+a)+"</div>");c.map(d.options.mapper,function(a){g=h.insertCell(-1);g.className="ui-widget ui-widget-content";g.style.cssText="text-align:"+f[g.cellIndex].style.textAlign;c.data(g,"cell-json",b);c(g).html(c.isFunction(a.render)?a.render.call(g,b[a.name],b):b[a.name])})});f=j=h=g=d=a=null},_ajax:function(){var a=this.options,b=a.jsonStore.url,e=a.limit,d=this._offset,f=a.jsonStore;i===b||""===b?(b=(f.data.rows||f.data)[0],i===b||i===b[a.mapper[0].name]?alert("Invalid JSON"):this._createRows(f.data,
"local"),b=null):("string"===typeof f.params?f.params=0===d?f.params+"&limit="+e+"&offset="+d:f.params.replace(/(&offset=)(.+)/,"&offset="+d):(i===f.params&&(f.params={}),f.params.limit=e,f.params.offset=d),c.ajax({type:a.ajaxMethod.toLowerCase(),url:b.replace(/\?.*/,""),data:f.params,dataType:"json",context:this,success:function(a){if(i!=a.error||0===a.length)return c.isFunction(this.options.onError)?this.options.onError.call(this.element[0]):alert(0===this.length?"Empty rows":this.error),!1;this._createRows(a,
"ajax")}}))},render:function(){var a=this,b,e=0;if(a._active())a.resetOffset(),a.load();else{c.isArray(a.options.toolBarButtons)&&(b=a.uiDataGridTfoot[0].rows[0].cells[0],c.map(a.options.toolBarButtons,function(d){(function(a){if(c.isFunction(d.fn))this.on("click",function(b){b.preventDefault();d.fn.apply(a.element[0],arguments);c(this).blur();return!1});this.button({icons:{primary:i===d.icon?null:"ui-icon-"+d.icon}});b.appendChild(this[0])}).call(c("<button>").text(d.label),a)}),b=null);a.uiDataGrid.appendTo(a.element);
