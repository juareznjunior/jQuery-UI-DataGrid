...
this._offset=this._num_rows=0;this._createPagination();if("string"===typeof this.options.jsonStore.params||this.options.jsonStore.params instanceof String)this.options.jsonStore.params=this._queryStringToJSON(this.options.jsonStore.params)},_init:function(){!0===this.options.autoRender&&this.render()},_createColumns:function(){var a=this,c=null,d=null,e="<col></col>",f='<th class="ui-widget ui-state-default" role="columnheader"></th>',k='<td class="ui-widget ui-widget-content"></td>',l="ui-datagrid-align-",
m="ui-datagrid-column-hide",g=0,h=b([a.uiDataGridColGroup1[0],a.uiDataGridColGroup2[0],a.uiDataGridColGroup3[0]]).empty(),p=b([a.uiDataGridThead[0].rows[0],a.uiDataGridTheadBody[0].rows[0]]).empty(),n=100/a.options.mapper.length;a.uiDataGridLayout.empty();a.options.rowNumber&&(h.append(b("<col></col>").addClass("ui-datagrid-cell-rownumber")),p.append('<th class="ui-state-default ui-datagrid-cell-rownumber" role="columnheader"></th>'),a.uiDataGridLayout.append(k));b.map(a.options.mapper,function(g,
r){d=g.title||g.name;r=r<a.options.mapper.length-1?isNaN(parseFloat(g.width))?n+"%":g.width:"";c=b(f).html(d);c=c.text(c.text());c.data("text-align",l+(/left|right|center/.test(g.align)?g.align:"left")).addClass(function(){return b(this).data("textAlign")});!0===g.sort&&c.addClass("ui-datagrid-sort ui-state-disabled").html(function(a){return b("<button>"+this.innerHTML+"</button>",{type:"button"}).button({icons:{primary:"",secondary:"ui-icon-carat-2-n-s"}})});var s=b(k),t=b(e).width(r);void 0!==g.hidden&&
b([c.data("hidden",m)[0],t[0],s[0]]).addClass(m);p.append(c);h.append(t);a.uiDataGridLayout.append(s);t=s=g=null});a.uiDataGridColGroup1.children().map(function(){g+=b(this).width()});g>a.element.width()&&a.uiDataGridScrollMain.width(g);g=n=0;b(a.uiDataGridTheadBody[0].parentNode.tHead).hide();m=l=k=f=e=c=p=h=null;this._sort()},_createRows:function(a,c,d){var e=this,f=e.getThead()[0].rows[0].cells,k=d?e.uiDataGridTbody[0]:e.uiDataGridTbody.empty()[0],l=d?k.rows.length+1:e._offset+1,m=!d&&"local"===
c&&e.options.pagination,g,h;0===e._num_rows&&(e._num_rows=void 0===a.num_rows?void 0===a.rows?void 0===a[0].num_rows?a.length:a[0].num_rows:void 0===a.rows[0].num_rows?a.rows.length:a.rows[0].num_rows:a.num_rows);a=a.rows||a;m&&1<l&&(a=a.slice(e._offset));b.map(a,function(a,c){if(m&&c===e.options.limit)return!1;g=k.insertRow(-1);g.className="ui-state-hover";b(g).data("row-json",a).data("row-index",l+1);e.options.rowNumber&&b(g.insertCell(0)).addClass("ui-state-default ui-datagrid-cell-rownumber").text(l+
c);b.isFunction(e.options.eventController.onClickRow)&&b(g).data("uiDataGridBindClick","onClickRow");b.map(e.options.mapper,function(c,d){h=g.insertCell(-1);h.className="ui-widget ui-widget-content";b.map(b(f[h.cellIndex]).data(),function(a,b){/textAlign|text-align|hidden/.test(b)&&(h.className+=" "+a)});b(h).html(b.isFunction(c.render)?c.render.call(h,a[c.name]):a[c.name])})});d||(this._updatePagination(),e.uiDataGridScrollBody.scrollTop(0));b.isPlainObject(this.options.sort)&&b(this.uiDataGridThead[0].rows[0].cells).filter(".ui-datagrid-sort").removeClass("ui-state-disabled");
f=k=g=h=e=a=null},_createPagination:function(){if(!0===this.options.pagination){var a=this,c=b(this.uiDataGridTfoot[0].rows[0].cells).last()[0];a.uiDataGridTdPagination.childs.push(b(c).children()[0]);b.map(["first","prev","next","end"],function(d,e){e=b("<button></button>",{type:"button",name:"uiDataGridSetPage-"+d,disabled:!0}).text(d).button({icons:{primary:"ui-icon-seek-"+d},text:!1}).data("uiDataGridBindClick","loadPage").appendTo(c);a.uiDataGridTdPagination.childs.push(e[0])});a=c=null}},_updatePagination:function(){var a,
...
this.options.limit-this.options.limit},_firstPage:function(){this._offset=0},_active:function(){return this.element.children(":eq(0)").hasClass("ui-datagrid-container")},_getBHF:function(a,c){return b.isFunction(c)?c.call(a[0]):a},_message:function(a){this.uiDataGridTbody.empty();this.uiDataGridMsgError.text(a).addClass("show-message")},_ajax:function(){var a=this.options,c=a.limit,d=this._offset,e=a.jsonStore,f=e.url;b.isPlainObject(this.options.sort)&&b(this.uiDataGridThead[0].rows[0].cells).filter(".ui-datagrid-sort").addClass("ui-state-disabled");
this.clearSelectedRows();this.uiDataGridMsgError.removeClass("show-message");void 0===f||""===f?(f=(e.data.rows||e.data)[0],void 0===f||void 0===f[a.mapper[0].name]?this._message("Invalid JSON or empty data"):this._createRows(e.data,"local"),f=e=a=null):(e.params.limit=c,e.params.offset=d,a=a.ajaxMethod.toLowerCase(),b.ajax({type:a,url:f.replace(/\?.*/,""),data:e.params,dataType:"json",context:this,success:function(a){if(void 0!==a.error||0===a.length)return a=void 0!==a.error?a.error:0===a.length?
this.options.emptyDataMessage:"Invalid JSON",b.isFunction(this.options.onError)?this.options.onError.call(this.element[0],a):this._message(a),!1;this._createRows(a,"ajax");b.isFunction(this.options.onAjaxSuccess)&&this.options.onAjaxSuccess.call(this.element[0])}}),e=a=null)},_sort:function(){function a(a){a=a.cells.item(e).textContent.toLowerCase().replace(".","").replace(",",".");return!1===isNaN(a)?parseFloat(a):a}function c(b,c){var d=a(b),e=a(c),f=parseInt(d,10);f&&(d=f,e=parseInt(e,10));return d>
e?1:d<e?-1:0}function d(a){var d=this.parentNode;if(!b(d).hasClass("ui-state-disabled")){var f=a.data.self,g=d.parentNode,h=b.data(g,"currentRowSort");a=f.uiDataGridTbody[0];var p=0,n=b(d).hasClass("tablesorter-headerAsc")?"tablesorter-headerDesc":"tablesorter-headerAsc",q=document.createDocumentFragment();b([d,void 0!==h&&d!==h?h:[]]).removeClass("tablesorter-header").removeClass("tablesorter-headerAsc").removeClass("tablesorter-headerDesc");e=d.cellIndex;b.data(g,"currentRowSort",d);f.addParam("order="+
f.options.mapper[e].name+"_"+n.replace("tablesorter-header","").toLowerCase());if(!0===f.options.sort.remote&&f._num_rows>f.options.limit)f.load();else{f=a.rows;f=Array.prototype.sort.call(Array.prototype.slice.call(f,0),c);for("tablesorter-headerDesc"===n&&Array.prototype.reverse.call(f);a.firstChild;)a.removeChild(a.firstChild);for(;g=f[p++];)q.appendChild(g);a.appendChild(q)}b(d).addClass("tablesorter-header "+n)}}if(b.isPlainObject(this.options.sort)){var e,f=this.options.sort;/^[0-9]$/.test(parseInt(f.initial,
10).toString())&&(e=f.initial,b.data(this.uiDataGridThead[0].rows[0],"currentRowSort",b(this.uiDataGridThead[0].rows[0].cells[e]).addClass("tablesorter-header tablesorter-header"+(/^(Desc|Asc)$/.test(f.direction)?f.direction:"Asc"))[0]));this.uiDataGridThead.on("click.uiDataGridEventSort",".ui-button.ui-button-text-icon-secondary",{self:this},d)}},_eventObserve:function(a){this._on(this.uiDataGridTbody,{click:this._setupGridEvents});!0===this.options.pagination&&this._on(this.uiDataGridTfoot[0].rows[0].cells[1],
...
b.widget("ui.datagrid",{_setOption:function(a,c){var d,e;"jsonStore"===a&&b.isPlainObject(c)&&(d={url:" ",params:{},data:{}},e=this,b.each(c,function(a,k){d[a]&&(c[a]="params"===a?b.isPlainObject(k)?k:e._queryStringToJSON(k):k)}),c=b.extend(d,c),d=e=null);this._super(a,c);"title"===a&&this.uiDataGridScrollMain.parent().prev().children().html(c)},_destroy:function(){this.element.empty()}});b.widget("ui.datagrid",b.ui.datagrid,q);q=null});