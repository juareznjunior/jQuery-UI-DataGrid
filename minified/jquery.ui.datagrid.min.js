...
d+'<thead><tr><th><table class="ui-datagrid"><colgroup></colgroup><thead><tr></tr></thead></table></th><th style="width:17px"></th></tr></thead></table></div><div class="ui-widget-content ui-datagrid-body"><table class="ui-datagrid"><thead><tr></tr></thead><tbody></tbody></table></div>';if(this.options.pagination||b.isArray(this.options.toolBarButtons))c+='<div class="ui-widget ui-state-default ui-datagrid-tools"><table cellspacing="0" cellpadding="0" class="ui-datagrid"><tbody><tr><td>&nbsp;</td>',
this.options.pagination&&(c+="<td><span></span></td>"),c+="</tr></tbody></table></div>";c+="</div>";a.innerHTML=c;return a},_createToolButtons:function(){for(var a=[],c=["first","prev","next","end"],d,e,f=0;e=c[f++];)d=document.createElement("button"),d.name="data-grid-button-"+e,d.innerHTML=e,b(d).button({icons:{primary:"ui-icon-seek-"+e},text:false}),a[a.length]=d;b(a).appendTo(b(this.uiDataGridTfoot[0].rows[0].cells).eq(-1)[0]);return this},_disableToolButtons:function(){b(this.uiDataGridTfoot[0].rows[0].cells).eq(-1).children(":button").removeClass("ui-state-hover ui-state-focus").button("disable");
...
h=0}y=0},_ajax:function(){this.options.jsonStore.url!=""?("string"===typeof this.options.jsonStore.params?this.options.jsonStore.params=0===this._offset?this.options.jsonStore.params+"&limit="+this.options.limit+"&offset="+this._offset:this.options.jsonStore.params.replace(/(&offset=)(.+)/,"&offset="+this._offset):(this.options.jsonStore.params.limit=this.options.limit,this.options.jsonStore.params.offset=this._offset),this.options.pagination&&this._disableToolButtons(),b.ajax({type:this.options.ajaxMethod.toLowerCase(),
url:this.options.jsonStore.url.replace(/\?.*/,""),data:this.options.jsonStore.params,dataType:"json",context:this,success:function(a){var c=this;if(void 0!=a.error)return alert(a.error),b.isFunction(c.options.onError)&&c.options.onError.call(c.element[0]),false;if(void 0===a.num_rows||a.num_rows==0)return false;if(c.options.pagination){c._totalPages=Math.ceil(a.num_rows/c.options.limit);var d=c._offset==0?1:c._offset/c.options.limit+1,e=d+" de "+c._totalPages+" ("+a.num_rows+")";b.each(b(c.uiDataGridTfoot[0].rows[0].cells).eq(-1).children(),
function(){/span/i.test(this.tagName)?this.innerHTML=e:/data-grid-button-(first|prev)/.test(this.name)?c._offset>0&&this.disabled&&b(this).button("enable"):c._totalPages>d&&this.disabled&&b(this).button("enable")})}c._createRows(a);c=null}})):this._createRows(this.options.jsonStore.data)},render:function(){var a=this;if(a._active())a.resetOffset(),a.load();else{if(b.isArray(a.options.toolBarButtons)){var c=a.uiDataGridTfoot[0].rows[0].cells[0];b.each(a.options.toolBarButtons,function(e,d){(function(a){b.isFunction(d.fn)&&
this.bind("click",function(){d.fn.apply(a.element[0],arguments);b(this).blur()});this.button({icons:{primary:void 0===d.icon?null:"ui-icon-"+d.icon}});c.appendChild(this[0])}).call(b(document.createElement("button")).html(d.label),a)});c=null}a.uiDataGrid.appendTo(a.element);a._createColumns();var d=a.uiDataGridThead.outerHeight();a.uiDataGridTheadBody.parent().css("marginTop",-d);d=null;a.options.pagination&&(a._createToolButtons()._disableToolButtons(),b(a.uiDataGridTfoot[0].rows[0].cells).eq(-1).delegate("button",
"click",function(a){return function(){if(false===this.disabled){var b=["_",this.name.replace(/data-grid-button-/,""),"Page"];a[b.join("")]();a._selectedRows=[];a.load()}}}(a)));a.resize();a.options.autoLoad&&a.load();b.isFunction(a.options.onComplete)&&function(a){setTimeout(function(){a.options.onComplete.call(a.element[0])})}(a)}a=null;return this},_nextPage:function(){this._offset+=this.options.limit},_prevPage:function(){this._offset-=this.options.limit},_endPage:function(){this._offset=this._totalPages*
this.options.limit-this.options.limit},_firstPage:function(){this._offset=0},_tbodyEvents:function(){b.isFunction(this.options.onClickRow)&&this.uiDataGridTbody.undelegate().delegate("tr","click",function(a){return function(c){b(this).addClass("ui-state-highlight");a.options.onClickRow.call(a,this);var d=a.uiDataGridTbody[0],e=b.data(d,"rowselected");c.currentTarget!==e[0]&&e.removeClass("ui-state-highlight");b.data(d,"rowselected",b(c.currentTarget))}}(this));return this},_active:function(){return this.element.children(":eq(0)").hasClass("ui-datagrid-container")},
