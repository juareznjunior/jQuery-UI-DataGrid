...
;(function( factory ) {
   ...
   var jQueryDataGrid = {
      ...
      ,_observer: function() {
         ...
         this.element.on('click.uiDataGridBindClick',function(event){
            ...
            if ( undefined === bindClick ) {
              bindClick = $(target.parentNode).data('uiDataGridBindClick');
            ...
            // event obeserver
            // nao afeta a[href] sem data bind
            if ( undefined !== bindClick ) {
               uiDataGrid = $(this).data('ui-datagrid');

               // internal event call
               if ( 'loadPage' === bindClick ) {
                  if ( false === target.disabled ) {
                     trigger = $(uiDataGrid.uiDataGridTdPagination.childs).filter('button');
                     trigger.removeClass('ui-state-hover ui-state-focus').button('disable');
                     trigger = target.name.split('-').slice(-1);
                     uiDataGrid['_'+trigger+'Page']();
                     uiDataGrid.load();
                  }
               } else { // options event call

                  // default
                  trigger = uiDataGrid.options.eventController;

                  // toolBarButtons
                  if ( /^toolBarButtons/.test(bindClick) ) {
                     trigger = uiDataGrid.options.toolBarButtons;
                     bindClick = bindClick.replace('toolBarButtons:','');
                  }
            ...
            bindClick = trigger = uiDataGrid = target = null;
         });
      }
      ,render: function() {
